...
class Products {
  async getProducts() {
    try {
      ...
      data = data.map((item) => {
        const { title, price, type,packaging, rating } = item.fields;
        const id = item.sys.id;
        const url = item.fields.image.fields.file.url;
        return { title, price,type,packaging,rating, id, url };
      });
      return data;
    } 
    catch (error)
      ...
      result += `
            <article class="product">
                ...
                <div>
                    ...
                    <h4 class="pack">$ ${element.packaging}</h4>
                </div>
                <div class="star">
                   ${star}
                </div>
